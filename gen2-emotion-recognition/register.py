...
def photo_http():
    ...
    _, encimg = cv2.imencode(".png", photo)
    ...
    #リクエスト処理
    req = urllib.request.Request(url, data=img_json, method='PUT', headers=req_header)
    try:
...
# thread start
id_name = args.name
fr_thread = threading.Thread(target=face_rec, args=(id_name,))
fr_thread.start()

rg_thread = threading.Thread(target=register_guide)
rg_thread.start()

ph_thread = threading.Thread(target=photo_http)
ph_thread.start()
